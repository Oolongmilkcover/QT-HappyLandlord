    ...
    , ui(new Ui::ScorePanel)
{
    ...
    m_list<<ui->meScore<<ui->leftScore<<ui->rightScore
           <<ui->myTitile<<ui->leftTitile<<ui->rightTitle
...
{
    QString style = QString("QLabel {color:}").arg(MYCOLOR[color]);
    for(int i = 0;i<m_list.size();i++){
        m_list[i]->setStyleSheet(style);
    }
}

void ScorePanel::setLeftName(const QString &name)
{
    ui->leftTitile->setText(name+":");
}

void ScorePanel::setRightName(const QString &name)
{
    ui->rightTitle->setText(name+":");
}

void ScorePanel::setmiddleName(const QString &name)
{
    ui->myTitile->setText(name+":");
}
