...
void GamePanel::GameControlInit()
{
    ...
    connect(m_gameCtl,&GameControl::playerStatusChanged,this,&GamePanel::onPlayStatusChanged);
...
void GamePanel::gameStatusPrecess(GameControl::GameStatus status)
{
    ...
    case GameControl::DispatchCard:
        m_gameCtl->getLeftRobot()->setGameOver(false);
        m_gameCtl->getRightRobot()->setGameOver(false);
        //播放背景音乐
        m_bgm->startBGM();
...
void GamePanel::onDispatchCard()
{
    ...
    //移动扑克牌
    cardMoveStep(curPlayer,curMovePos);
    curMovePos = qMin(curMovePos + 15, 100);
}

void GamePanel::onPlayStatusChanged(Player *player, GameControl::PlayerStatus status)
{
    switch(status){
...
void GamePanel::onUserPlayHand()
{
    ...
    //判断玩家是不是用户
    if(m_gameCtl->getUserPlayer()!=m_gameCtl->getUserPlayer()){
        return;
    }
    //判断要出的牌是否为空
...
{
    bool isLoard =  m_gameCtl->getUserPlayer()->getRole() == Player::Lord ? true:false;
